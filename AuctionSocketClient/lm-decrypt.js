...
function ShowDialog(title, width, height) {
    var dialogorder = $("#testdialog").dialog({
        ...
        buttons: [{
            ...
            click: function() {
                $(this).dialog("close");
                ShowProtocolDialog("拍卖须知", 700, 470);
...
function funCheckBrowser() {
    ...
    var s;
    var scan; (s = ua.match(/msie ([\d.]+)/)) ? Sys.ie = s[1].split(".")[0] : (s = ua.match(/edge\/([\d.]+)/)) ? Sys.edge = s[1].split(".")[0] : (s = ua.match(/firefox\/([\d.]+)/)) ? Sys.firefox = s[1].split(".")[0] : (s = ua.match(/chrome\/([\d.]+)/)) ? Sys.chrome = s[1].split(".")[0] : (s = ua.match(/opera.([\d.]+)/)) ? Sys.opera = s[1].split(".")[0] : (s = ua.match(/version\/([\d.]+).*safari/)) ? Sys.safari = s[1].split(".")[0] : 0;
    var comment = "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;请在Windows视窗系统上使用ie7及以上版本浏览器或在Mac OS X系统上使用Safari浏览器。";
    if (Sys.ie) {
